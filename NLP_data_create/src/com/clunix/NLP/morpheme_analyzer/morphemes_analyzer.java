...
public class morphemes_analyzer
{
	public List<String> analyzer_space(String str, int n) throws IOException, InterruptedException
	{
		...
		str = str.replaceAll("\\`", "\\\\\\`");
		String command = "echo \"" + str.trim() + "\" | " + "mecab -b 1024000 -N " + Integer.toString(n) + " -d /usr/local/lib/mecab/dic/mecab-ko-dic/ &> "
				+ dir + tmp_file;
		bw_sh.write(command);
		bw_sh.close();
		...
		if (str.contains("게/EC")) {
			...
			for (int i = 0; i < arr.length; i++) {
				if (arr[i].equals("게/EC")) {
					if (i > 0 && (arr[i - 1].contains("/VA") || arr[i - 1].contains("/XSA") || arr[i - 1].contains("/VCN") || arr[i - 1].contains("/VCP"))) { // 151001
																																								// modify
						arr[i] = "게/JKB";
					}
				}
				...
				if (posarr[4].equals("Inflect")) {
					...
					if (tarr.length == 1) {
						...
						for (int i2 = 0; i2 < tarr.length; i2++) {
							...
							if (tarr[i2].endsWith(("/"))) { // mecab-ko-dic
															// update
	...
	public static String phonetic_reduced(String str)
	{
		...
		if (tmp.contains("/J") || tmp.contains("/E")) {
			if (tmp.contains(" 은")) {
				...
				tmp = tmp.replaceAll("으면서", "면서으면서");
			}
			else if (tmp.contains("면서")) {
				tmp = tmp.replaceAll("면서", "면서으면서");
			}
