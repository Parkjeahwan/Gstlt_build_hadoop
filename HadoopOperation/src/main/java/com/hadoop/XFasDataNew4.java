...
public class XFasDataNew4 extends Configured implements Tool
{
	public static class NFasDataNew_Mapper extends Mapper<Object, Text, Text, LongWritable> {
		...
		public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
			...
			for (int i=0;i<m.length;i++) {
				...
				if(i < m.length - 1){
					key2 = m[i] + "##SP" + m[i+1];
					if (tested.contains(key2)) continue;
					else tested.add(key2);
	...
	public static void main(String[] args) throws Exception {
		int res = ToolRunner.run(new Configuration(), new XFasDataNew4(), args);
		System.exit(res);
	}
	...
	public int run(String[] args) throws Exception
	{
		...
		job.setJarByClass(XFasDataNew4.class);
