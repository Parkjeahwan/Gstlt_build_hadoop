...
public class Gstlt_build extends Configured implements Tool {
	...
	public static class Gstlt_build_Mapper extends Mapper<Text, Object, Text, Object> {
		...
		public boolean NNGLV_2(String morp) {
		...
		public boolean one_morpheme(String str){	
		...
		public void findGstalt(String line,Node np, Node norip, SGraph g) throws InterruptedException { // 문장 스트링 line을 받아서 입력 서술어 np에 연결되는 연속체를 붙여 g에 넣어 돌려주는 함수
			Node nje,nb;
			nje=nb= null;
			String M[] = line.split(" ");
		...
		public void addGstalt(Node nj,Node np, SGraph g) throws InterruptedException {
			g.SUCC.get(nj.ID).addNext(np);
			g.PREV.get(np.ID).addNext(nj);
		}

		public int offsetOfP(String line,String P) {
			String []a = P.split(" ");
			int len = a.length; 
		...
		public void addGstalt(Node nb,Node nj,Node np, SGraph g) throws InterruptedException {
			Node nt = null;
		...
		public String NFPR(String pstr) {
			String ret="";
		...
		public String N_stem(String s){	// 체언 어간 151114 추가 함수 
			String str = s;
		...
		public String P_stem(String str) {    //형태소 분석된 문장(일부)를 받아서 가장 후위의 용언 어간을 리턴하는 함수
			// pi 는 용언 어절의 index
			// pp 는 용언 형태소
			String pp ="";
			String[] arr = str.split(" ");       
		...
		public SGraph Gstlt_utils(SGraph g, String line) throws InterruptedException{
